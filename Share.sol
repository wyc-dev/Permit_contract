...
contract Share is ERC20, ReentrancyGuard {
    ...
    error OngoingProposalNotDeadlineYet();
    error MerchantAlreadyExists();
    error PercentageOutOfRange();
    error NoOngoingAddProposal();
    error ProposalExpired();
    error AlreadyVotedInThisProposal();
    error NoOngoingModProposal();
    error NoOngoingChangeProposal();
    error NoOngoingWithdrawProposal();
    error TotalSupplyZero();
    error TransfersLockedDuringActiveProposals();
    ...
    function initiateAdd(uint256 printQuota, address merchantAddr, string memory merchantName) external nonReentrant {
        ...
        if (isAnyProposalActive()) revert OngoingProposalNotDeadlineYet();
    ...
    function initiateChange(uint256 newPercentage) external nonReentrant {
        if (balanceOf(_msgSender()) == 0) revert MustHoldShareTokens();
        if (isAnyProposalActive()) revert OngoingProposalNotDeadlineYet();
        if (newPercentage == 0 || newPercentage > 30) revert PercentageOutOfRange();
    ...
    function voteAdd() external nonReentrant {
        if (!recentAdd.voting) revert NoOngoingAddProposal();
        if (block.timestamp > recentAdd.deadline) revert ProposalExpired();
        if (balanceOf(_msgSender()) == 0) revert MustHoldShareTokens();
        if (hasVotedAdd[currentAddID][_msgSender()]) revert AlreadyVotedInThisProposal();
    ...
    function voteMod() external nonReentrant {
        if (!recentMod.voting) revert NoOngoingModProposal();
        if (block.timestamp > recentMod.deadline) revert ProposalExpired();
        if (balanceOf(_msgSender()) == 0) revert MustHoldShareTokens();
        if (hasVotedMod[currentModID][_msgSender()]) revert AlreadyVotedInThisProposal();
    ...
    function voteChange() external nonReentrant {
        if (!recentChange.voting) revert NoOngoingChangeProposal();
        if (block.timestamp > recentChange.deadline) revert ProposalExpired();
        if (balanceOf(_msgSender()) == 0) revert MustHoldShareTokens();
        if (hasVotedChange[currentChangeID][_msgSender()]) revert AlreadyVotedInThisProposal();
    ...
    function voteWithdraw() external nonReentrant {
        if (!recentWithdraw.voting) revert NoOngoingWithdrawProposal();
        if (block.timestamp > recentWithdraw.deadline) revert ProposalExpired();
        if (balanceOf(_msgSender()) == 0) revert MustHoldShareTokens();
        if (hasVotedWithdraw[currentWithdrawID][_msgSender()]) revert AlreadyVotedInThisProposal();
    ...
    /**
     * @notice Internal function to check and execute the 我需要修復 Share.sol 中的問題。

問題1: ETH transfer 使用 transfer，可能 gas 不足。線 498 是 payable(initiator).transfer(address(this).balance);

修復: 使用 call。

從工具: 建議使用 address.call{value: amount}("")，並檢查 success。

也提到 OpenZeppelin Address.sendValue，但合約未 import Address，可添加 import {Address} from "@openzeppelin/contracts/utils/Address.sol";

然後使用 Address.sendValue(initiator, address(this).balance);

這處理 reentrancy。

問題2: Pragma ^0.8.27 改固定 0.8.27。

問題3: 替換 require 為 custom errors。

定義 errors 如 error MustHoldShareTokens();

然後 revert MustHoldShareTokens();

這節省 gas。

問題4: 移除 default 初始化，如 uint256 actualPrintQuota = 0; 改 uint256 actualPrintQuota;

問題5: 函數 visibility 改 private，如 _checkAndExecuteAdd 等。

問題6: 事件加 indexed。

例如 event ProposalInitiated(string indexed proposalType, uint256 indexed id, address indexed initiator);

問題7: 加 @custom:security-contact

用戶未指定，假設用 placeholder 如 @custom:security-contact security@example.com

問題8: Mapping 加 named，如 mapping(uint256 proposalId => mapping(address voter => bool)) public hasVotedAdd;

現在，輸出修復合約，使用 NatSpec 格式。```solidity
// SPDX-License-Identifier: MIT
...
contract Share is ERC20, ReentrancyGuard {
    ...
    error ProposalExpired(string proposalType);
    error AlreadyVoted(string proposalType);
    error TotalSupplyZero();
    ...
    function voteAdd() external nonReentrant {
        ...
        if (block.timestamp > recentAdd.deadline) revert ProposalExpired("Add");
        if (balanceOf(_msgSender()) == 0) revert MustHoldShareTokens();
        if (hasVotedAdd[currentAddID][_msgSender()]) revert AlreadyVoted("Add");
    ...
    function voteMod() external nonReentrant {
        ...
        if (block.timestamp > recentMod.deadline) revert ProposalExpired("Mod");
        if (balanceOf(_msgSender()) == 0) revert MustHoldShareTokens();
        if (hasVotedMod[currentModID][_msgSender()]) revert AlreadyVoted("Mod");
    ...
    function voteChange() external nonReentrant {
        ...
        if (block.timestamp > recentChange.deadline) revert ProposalExpired("Change");
        if (balanceOf(_msgSender()) == 0) revert MustHoldShareTokens();
        if (hasVotedChange[currentChangeID][_msgSender()]) revert AlreadyVoted("Change");
    ...
    function voteWithdraw() external nonReentrant {
        ...
        if (block.timestamp > recentWithdraw.deadline) revertProposalExpired("Withdraw");
        if (balanceOf(_msgSender()) == 0) revert MustHoldShareTokens();
        if (hasVotedWithdraw[currentWithdrawID][_msgSender()]) revert AlreadyVoted("Withdraw");
     ...
     * @dev Called after votes; withdraws tokens/ETH to initiator using safe transfer.
     */
